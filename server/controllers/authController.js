...
export const updateAdmin = async (req, res) => {
    const { id } = req.params
    const { username, password, role_id } = req.body
    const { role_id: userRole } = req.user
    ...
    try {
        ...
        if (role_id && userRole !== 1) {
            return res
                .status(403)
                .json({ message: 'No autorizado para cambiar roles' })
        }
...
export const getAllAdmins = async (req, res) => {
    ...
    if (userRole !== 1) {
        return res
            .status(403)
            .json({ message: 'No autorizado para ver administradores' })
    }
...
export const getAdminById = async (req, res) => {
    ...
    if (userRole !== 1 && userId !== parseInt(id)) {
        return res
            .status(403)
            .json({ message: 'No autorizado para ver este administrador' })
    }
